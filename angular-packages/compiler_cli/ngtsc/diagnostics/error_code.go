...
const (
	ErrorCode_DECORATOR_ARG_NOT_LITERAL ErrorCode = 1001
	ErrorCode_DECORATOR_ARITY_WRONG     ErrorCode = 1002
	ErrorCode_DECORATOR_NOT_CALLED      ErrorCode = 1003
	ErrorCode_DECORATOR_UNEXPECTED      ErrorCode = 1005

	/**
	 * This error code indicates that there are incompatible decorators on a type or a class field.
	 */
	ErrorCode_DECORATOR_COLLISION ErrorCode = 1006

	ErrorCode_VALUE_HAS_WRONG_TYPE ErrorCode = 1010
	ErrorCode_VALUE_NOT_LITERAL    ErrorCode = 1011

	ErrorCode_DUPLICATE_DECORATED_PROPERTIES ErrorCode = 1012
	 ...
	 */
	ErrorCode_INITIALIZER_API_WITH_DISALLOWED_DECORATOR ErrorCode = 1050
	 ...
	 */
	ErrorCode_INITIALIZER_API_DECORATOR_METADATA_COLLISION ErrorCode = 1051

	/**
	 * Raised whenever an initializer API does not support the `.required`
	 * function, but is still detected unexpectedly.
	 */
	ErrorCode_INITIALIZER_API_NO_REQUIRED_FUNCTION ErrorCode = 1052

	/**
	 * Raised whenever an initializer API is used on a class member
	 * and the given access modifiers (e.g. `private`) are not allowed.
	 */
	ErrorCode_INITIALIZER_API_DISALLOWED_MEMBER_VISIBILITY ErrorCode = 1053

	/**
	 * Raised whenever there are duplicate binding property names for outputs, inputs & models.
	 */
	ErrorCode_DUPLICATE_BINDING_NAME ErrorCode = 1054

	/**
	 * An Angular feature, like inputs, outputs or queries is incorrectly
	 * declared on a static member.
	 */
	ErrorCode_INCORRECTLY_DECLARED_ON_STATIC_MEMBER ErrorCode = 1100

	ErrorCode_COMPONENT_MISSING_TEMPLATE ErrorCode = 2001
	ErrorCode_PIPE_MISSING_NAME          ErrorCode = 2002
	ErrorCode_PARAM_MISSING_TOKEN        ErrorCode = 2003
	ErrorCode_DIRECTIVE_MISSING_SELECTOR ErrorCode = 2004

	/** Raised when an undecorated class is passed in as a provider to a module or a directive. */
	ErrorCode_UNDECORATED_PROVIDER ErrorCode = 2005

	/**
	 * Raised when a Directive inherits its constructor from a base class without an Angular
	 * decorator.
	 */
	ErrorCode_DIRECTIVE_INHERITS_UNDECORATED_CTOR ErrorCode = 2006

	/**
	 * Raised when an undecorated class that is using Angular features
	 * has been discovered.
	 */
	ErrorCode_UNDECORATED_CLASS_USING_ANGULAR_FEATURES ErrorCode = 2007

	/**
	 * Raised when an component cannot resolve an external resource, such as a template or a style
	 * sheet.
	 */
	ErrorCode_COMPONENT_RESOURCE_NOT_FOUND ErrorCode = 2008
	 ...
	 */
	ErrorCode_CONFLICTING_INPUT_TRANSFORM ErrorCode = 2020

	/** Raised when a component has both `styleUrls` and `styleUrl`. */
	ErrorCode_COMPONENT_INVALID_STYLE_URLS ErrorCode = 2021

	/**
	 * Raised when a type in the `deferredImports` of a component is not a component, directive or
	 * pipe.
	 */
	ErrorCode_COMPONENT_UNKNOWN_DEFERRED_IMPORT ErrorCode = 2022

	/**
	 * Raised when a `standalone: false` component is declared but `strictStandalone` is set.
	 */
	ErrorCode_NON_STANDALONE_NOT_ALLOWED ErrorCode = 2023

	/**
	 * Raised when a named template dependency isn't defined in the component's source file.
	 */
	ErrorCode_MISSING_NAMED_TEMPLATE_DEPENDENCY ErrorCode = 2024

	/**
	 * Raised if an incorrect type is used for a named template dependency (e.g. directive
	 * class used as a component).
	 */
	ErrorCode_INCORRECT_NAMED_TEMPLATE_DEPENDENCY_TYPE ErrorCode = 2025

	/**
	 * Raised for `@Component` fields that aren't supported in a selectorless context.
	 */
	ErrorCode_UNSUPPORTED_SELECTORLESS_COMPONENT_FIELD ErrorCode = 2026

	/**
	 * A component is using both the `animations` property and `animate.enter` or `animate.leave`
	 * in the template.
	 */
	ErrorCode_COMPONENT_ANIMATIONS_CONFLICT ErrorCode = 2027

	/**
	 * Raised when an `@Service` class is using constructor dependency injection.
	 */
	ErrorCode_SERVICE_CONSTRUCTOR_DI ErrorCode = 2028

	ErrorCode_SYMBOL_NOT_EXPORTED ErrorCode = 3001
	/**
	 * Raised when a relationship between directives and/or pipes would cause a cyclic import to be
	 * created that cannot be handled, such as in partial compilation mode.
	 */
	ErrorCode_IMPORT_CYCLE_DETECTED ErrorCode = 3003

	/**
	 * Raised when the compiler is unable to generate an import statement for a reference.
	 */
	ErrorCode_IMPORT_GENERATION_FAILURE ErrorCode = 3004

	ErrorCode_CONFIG_FLAT_MODULE_NO_INDEX                             ErrorCode = 4001
	ErrorCode_CONFIG_STRICT_TEMPLATES_IMPLIES_FULL_TEMPLATE_TYPECHECK ErrorCode = 4002
	ErrorCode_CONFIG_EXTENDED_DIAGNOSTICS_IMPLIES_STRICT_TEMPLATES    ErrorCode = 4003
	ErrorCode_CONFIG_EXTENDED_DIAGNOSTICS_UNKNOWN_CATEGORY_LABEL      ErrorCode = 4004
	ErrorCode_CONFIG_EXTENDED_DIAGNOSTICS_UNKNOWN_CHECK               ErrorCode = 4005
	ErrorCode_CONFIG_EMIT_DECLARATION_ONLY_UNSUPPORTED                ErrorCode = 4006

	/**
	 * Raised when a host expression has a parse error, such as a host listener or host binding
	 * expression containing a pipe.
	 */
	ErrorCode_HOST_BINDING_PARSE_ERROR ErrorCode = 5001

	/**
	 * Raised when the compiler cannot parse a component's template.
	 */
	ErrorCode_TEMPLATE_PARSE_ERROR ErrorCode = 5002

	/**
	 * Raised when an NgModule contains an invalid reference in `declarations`.
	 */
	ErrorCode_NGMODULE_INVALID_DECLARATION ErrorCode = 6001

	/**
	 * Raised when an NgModule contains an invalid type in `imports`.
	 */
	ErrorCode_NGMODULE_INVALID_IMPORT ErrorCode = 6002

	/**
	 * Raised when an NgModule contains an invalid type in `exports`.
	 */
	ErrorCode_NGMODULE_INVALID_EXPORT ErrorCode = 6003

	/**
	 * Raised when an NgModule contains a type in `exports` which is neither in `declarations` nor
	 * otherwise imported.
	 */
	ErrorCode_NGMODULE_INVALID_REEXPORT ErrorCode = 6004

	/**
	 * Raised when a `ModuleWithProviders` with a missing
	 * generic type argument is passed into an `NgModule`.
	 */
	ErrorCode_NGMODULE_MODULE_WITH_PROVIDERS_MISSING_GENERIC ErrorCode = 6005

	/**
	 * Raised when an NgModule exports multiple directives/pipes of the same name and the compiler
	 * attempts to generate private re-exports within the NgModule file.
	 */
	ErrorCode_NGMODULE_REEXPORT_NAME_COLLISION ErrorCode = 6006

	/**
	 * Raised when a directive/pipe is part of the declarations of two or more NgModules.
	 */
	ErrorCode_NGMODULE_DECLARATION_NOT_UNIQUE ErrorCode = 6007

	/**
	 * Raised when a standalone directive/pipe is part of the declarations of an NgModule.
	 */
	ErrorCode_NGMODULE_DECLARATION_IS_STANDALONE ErrorCode = 6008

	/**
	 * Raised when a standalone component is part of the bootstrap list of an NgModule.
	 */
	ErrorCode_NGMODULE_BOOTSTRAP_IS_STANDALONE ErrorCode = 6009
	 ...
	 * `NgModule.id`.
	 */
	ErrorCode_WARN_NGMODULE_ID_UNNECESSARY ErrorCode = 6100
	 ...
	 */
	ErrorCode_SCHEMA_INVALID_ELEMENT ErrorCode = 8001

	/**
	 * An element's attribute name failed validation against the DOM schema.
	 */
	ErrorCode_SCHEMA_INVALID_ATTRIBUTE ErrorCode = 8002

	/**
	 * No matching directive was found for a `#ref="target"` expression.
	 */
	ErrorCode_MISSING_REFERENCE_TARGET ErrorCode = 8003

	/**
	 * No matching pipe was found for a
	 */
	ErrorCode_MISSING_PIPE ErrorCode = 8004
	 ...
	 */
	ErrorCode_WRITE_TO_READ_ONLY_VARIABLE ErrorCode = 8005
	 ...
	 * ```
	 */
	ErrorCode_DUPLICATE_VARIABLE_DECLARATION ErrorCode = 8006
	 ...
	 */
	ErrorCode_SPLIT_TWO_WAY_BINDING ErrorCode = 8007

	/**
	 * A directive usage isn't binding to one or more required inputs.
	 */
	ErrorCode_MISSING_REQUIRED_INPUTS ErrorCode = 8008
	 ...
	 * ```
	 */
	ErrorCode_ILLEGAL_FOR_LOOP_TRACK_ACCESS ErrorCode = 8009
	 ...
	 * ```
	 */
	ErrorCode_INACCESSIBLE_DEFERRED_TRIGGER_ELEMENT ErrorCode = 8010
	 ...
	 * ```
	 */
	ErrorCode_DEFER_IMPLICIT_TRIGGER_MISSING_PLACEHOLDER ErrorCode = 8019
	 ...
	 * ```
	 */
	ErrorCode_DEFER_IMPLICIT_TRIGGER_INVALID_PLACEHOLDER ErrorCode = 8020
	 ...
	 * ```
	 */
	ErrorCode_INVALID_BANANA_IN_BOX ErrorCode = 8101
	 ...
	 */
	ErrorCode_NULLISH_COALESCING_NOT_NULLABLE ErrorCode = 8102

	/**
	 * A known control flow directive (e.g. `*ngIf`) is used in a template,
	 * but the `CommonModule` is not imported.
	 */
	ErrorCode_MISSING_CONTROL_FLOW_DIRECTIVE ErrorCode = 8103
	 ...
	 */
	ErrorCode_TEXT_ATTRIBUTE_NOT_BINDING ErrorCode = 8104
	 ...
	 * ```
	 */
	ErrorCode_MISSING_NGFOROF_LET ErrorCode = 8105
	/**
	 ...
	 */
	ErrorCode_SUFFIX_NOT_SUPPORTED ErrorCode = 8106
	 ...
	 * When the type of foo doesn't include `null` or `undefined`.
	 */
	ErrorCode_OPTIONAL_CHAIN_NOT_NULLABLE ErrorCode = 8107
	 ...
	 * value
	 */
	ErrorCode_SKIP_HYDRATION_NOT_STATIC ErrorCode = 8108
	 ...
	 * ```
	 */
	ErrorCode_INTERPOLATED_SIGNAL_NOT_INVOKED ErrorCode = 8109
	 ...
	 * ```
	 */
	ErrorCode_UNSUPPORTED_INITIALIZER_API_USAGE ErrorCode = 8110
	 ...
	 * `<button (click)="myFunc()"></button>`.
	 */
	ErrorCode_UNINVOKED_FUNCTION_IN_EVENT_BINDING ErrorCode = 8111
	 ...
	 * ```
	 */
	ErrorCode_UNUSED_LET_DECLARATION ErrorCode = 8112

	/**
	 * A symbol referenced in `@Component.imports` isn't being used within the template.
	 */
	ErrorCode_UNUSED_STANDALONE_IMPORTS ErrorCode = 8113

	/**
	 * An expression mixes nullish coalescing and logical and/or without parentheses.
	 */
	ErrorCode_UNPARENTHESIZED_NULLISH_COALESCING ErrorCode = 8114
	 ...
	 * ```
	 */
	ErrorCode_UNINVOKED_TRACK_FUNCTION ErrorCode = 8115

	/**
	 * A structural directive is used in a template, but the directive is not imported.
	 */
	ErrorCode_MISSING_STRUCTURAL_DIRECTIVE ErrorCode = 8116
	 ...
	 * ```
	 */
	ErrorCode_UNINVOKED_FUNCTION_IN_TEXT_INTERPOLATION ErrorCode = 8117
	 ...
	 *  }
	 */
	ErrorCode_FORBIDDEN_REQUIRED_INITIALIZER_INVOCATION ErrorCode = 8118

	/**
	 * The template type-checking engine would need to generate an inline type check block for a
	 * component, but the current type-checking environment doesn't support it.
	 */
	ErrorCode_INLINE_TCB_REQUIRED ErrorCode = 8900

	/**
	 * The template type-checking engine would need to generate an inline type constructor for a
	 * directive or component, but the current type-checking environment doesn't support it.
	 */
	ErrorCode_INLINE_TYPE_CTOR_REQUIRED ErrorCode = 8901

	/**
	 * An injectable already has a `ɵprov` property.
	 */
	ErrorCode_INJECTABLE_DUPLICATE_PROV ErrorCode = 9001
	 ...
	 */
	ErrorCode_SUGGEST_STRICT_TEMPLATES ErrorCode = 10001
	 ...
	 * type inference.
	 */
	ErrorCode_SUGGEST_SUBOPTIMAL_TYPE_INFERENCE ErrorCode = 10002
	 ...
	 */
	ErrorCode_LOCAL_COMPILATION_UNRESOLVED_CONST ErrorCode = 11001
	 ...
	 */
	ErrorCode_LOCAL_COMPILATION_UNSUPPORTED_EXPRESSION ErrorCode = 11003
)
