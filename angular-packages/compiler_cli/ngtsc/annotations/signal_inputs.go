...
// parseModelInput checks if the initializer is a call to model() or model.required()
// and extracts the corresponding R3InputMetadata.
func parseModelInput(initializer *ast.Node, propName string) (render3.R3InputMetadata, bool) {
	if initializer == nil || initializer.Kind != ast.KindCallExpression {
		return render3.R3InputMetadata{}, false
	}
	callExpr := initializer.AsCallExpression()
	...
	if expr.Kind == ast.KindIdentifier && expr.AsIdentifier().Text == "model" {
		isModelInput = true
	} else if expr.Kind == ast.KindPropertyAccessExpression {
		pae := expr.AsPropertyAccessExpression()
		if pae.Expression.Kind == ast.KindIdentifier && pae.Expression.AsIdentifier().Text == "model" {
			if pae.Name() != nil && pae.Name().AsIdentifier().Text == "required" {
				isModelInput = true
				isRequired = true
			}
	...
	// Parse alias from options:
	// For model.required(options?), options is at index 0
	// For model(initialValue?, options?), options is at index 1
	var optionsNode *ast.Node
	if isRequired {
		if callExpr.Arguments != nil && len(callExpr.Arguments.Nodes) > 0 {
			optionsNode = callExpr.Arguments.Nodes[0]
