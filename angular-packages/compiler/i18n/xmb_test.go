...
func TestXmbSerializer(t *testing.T) {
	const html = `
...
`
	...
	t.Run("should write a valid xmb file", func(t *testing.T) {
		if got := toXmb(html, "file.ts", nil); got != xmb {
			t.Fatalf("xmb mismatch\nGOT:\n%s\nWANT:\n%s", got, xmb)
		}
		fr := "fr"
		if got := toXmb(html, "file.ts", &fr); got != xmb {
			t.Fatalf("xmb with locale mismatch\nGOT:\n%s\nWANT:\n%s", got, xmb)
		}
	})

	t.Run("should throw when trying to load an xmb file", func(t *testing.T) {
		defer func() {
			got := recover()
			if got == nil {
		...
		serializer := i18n.NewXmb()
		serializer.Load(xmb, "url")
	})
}

func toXmb(html string, url string, locale *string) string {
	catalog := i18n.NewMessageBundle(ml_parser.NewHtmlParser(), []string{}, map[string][]string{}, locale)
	serializer := i18n.NewXmb()
	catalog.UpdateFromTemplate(html, url, nil)
