...
func getHtmlTagDef(tagName string) tags.TagDefinition {
	return GetHtmlTagDefinition(tagName)
}

func tokenizeWithoutErrors(input string, options *TokenizeOptions) *TokenizeResult {
	res := Tokenize(input, "someUrl", getHtmlTagDef, options)
	if len(res.Errors) > 0 {
		var msgs []string
...
func humanizeParts(tokens []Token) [][]any {
	var res [][]any
	for _, token := range tokens {
		parts := []any{token.Type}
		for _, p := range token.Parts {
...
func tokenizeAndHumanizeParts(input string, options *TokenizeOptions) [][]any {
	return humanizeParts(Tokenize(input, "someUrl", getHtmlTagDef, options).Tokens)
}

func tokenizeAndHumanizeSourceSpans(input string, options *TokenizeOptions) [][]any {
	var res [][]any
	for _, token := range Tokenize(input, "someUrl", getHtmlTagDef, options).Tokens {
		res = append(res, []any{token.Type, token.SourceSpan.ToString()})
	}
	return res
}
...
func tokenizeAndHumanizeLineColumn(input string, options *TokenizeOptions) [][]any {
	var res [][]any
	for _, token := range Tokenize(input, "someUrl", getHtmlTagDef, options).Tokens {
		res = append(res, []any{token.Type, humanizeLineColumn(token.SourceSpan.Start)})
	}
	return res
}

func tokenizeAndHumanizeFullStart(input string, options *TokenizeOptions) [][]any {
	var res [][]any
	for _, token := range Tokenize(input, "someUrl", getHtmlTagDef, options).Tokens {
		res = append(res, []any{token.Type, humanizeLineColumn(token.SourceSpan.Start), humanizeLineColumn(token.SourceSpan.FullStart)})
	}
	return res
}

func tokenizeAndHumanizeErrors(input string, options *TokenizeOptions) [][]any {
	var res [][]any
	for _, e := range Tokenize(input, "someUrl", getHtmlTagDef, options).Errors {
		fmt.Printf("DEBUG error: %v\n", e.Msg)
		res = append(res, []any{e.Msg, humanizeLineColumn(e.Span.Start)})
	}
	fmt.Printf("DEBUG res length: %d\n", len(res))
	return res
}
...
options := &TokenizeOptions{SelectorlessEnabled: true}
...
assert.Equal(t, [][]any{
    ...
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts("<MyComp>hello</MyComp>", options))
})
...
assert.Equal(t, [][]any{
    ...
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts("<MyComp:button>hello</MyComp:button>", options))
})
...
assert.Equal(t, [][]any{
    ...
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts("<MyComp/>", options))
})
...
assert.Equal(t, [][]any{
    ...
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts("<div @MyDir></div>", options))
})
...
assert.Equal(t, [][]any{
    ...
    []any{TokenTypeTagClose, "", "div"},
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts("<div @MyDir(foo)></div>", options))
})
t.Run("should parse a directive with attributes", func(t *testing.T) {
tokens := tokenizeAndHumanizeParts("<div @MyDir(static=\"one\" [bound]=\"expr\" [(twoWay)]=\"expr\" #ref=\"name\" (click)=\"handler()\")></div>", options)
assert.Equal(t, [][]any{
    []any{TokenTypeTagOpenStart, "", "div"},
    []any{TokenTypeDirectiveName, "MyDir"},
    []any{TokenTypeDirectiveOpen},
...
tokens := tokenizeAndHumanizeParts("<div before=\"value\" @OneDir([one]=\"1\" two=\"2\") middle @TwoDir @ThreeDir((three)=\"handleThree()\") after=\"value\"></div>", options)
assert.Equal(t, [][]any{
    []any{TokenTypeTagOpenStart, "", "div"},
...
assert.Equal(t, [][]any{
    ...
    []any{TokenTypeTagClose, "", "div"},
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts("<div hello=\"@MyDir\"></div>", options))
})
t.Run("should produce spans for directives", func(t *testing.T) {
tokens := tokenizeAndHumanizeSourceSpans("<div @Empty @NoAttrs() @WithAttr([one]=\"1\" two=\"2\") @WithSimpleAttr(simple)></div>", options)
assert.Equal(t, [][]any{
    []any{TokenTypeTagOpenStart, "<div"},
...
tokens := tokenizeAndHumanizeSourceSpans("<div    @Dir   (  one=\"1\"    (two)=\"handleTwo()\"     )     ></div>", options)
assert.Equal(t, [][]any{
    []any{TokenTypeTagOpenStart, "<div"},
...
result := tokenizeWithoutErrors("{\r\n    messages.length,\r\n    plural,\r\n    =0 {You have \r\nno\r\n messages}\r\n    =1 {One {{message}}}}\r\n", &TokenizeOptions{TokenizeExpansionForms: true, EscapedString: true, I18nNormalizeLineEndingsInICUs: true})
assert.Equal(t, [][]any{
    []any{TokenTypeExpansionFormStart},
...
assert.Equal(t, []Token{}, result.NonNormalizedIcuExpressions)
})
...
result := tokenizeWithoutErrors("{\r\n    messages.length,\r\n    plural,\r\n    =0 {You have \r\nno\r\n messages}\r\n    =1 {One {{message}}}}\r\n", &TokenizeOptions{TokenizeExpansionForms: true, EscapedString: true})
assert.Equal(t, [][]any{
    []any{TokenTypeExpansionFormStart},
    []any{TokenTypeRawText, "\r\n    messages.length"},
    []any{TokenTypeRawText, "plural"},
...
t.Run("should normalize line-endings in expansion forms if `i18nNormalizeLineEndingsInICUs` is true", func(t *testing.T) {
result := tokenizeWithoutErrors("{\r\n    messages.length,\r\n    plural,\r\n    =0 {You have \r\nno\r\n messages}\r\n    =1 {One {{message}}}}\r\n", &TokenizeOptions{TokenizeExpansionForms: true, EscapedString: false, I18nNormalizeLineEndingsInICUs: true})
assert.Equal(t, [][]any{
    []any{TokenTypeExpansionFormStart},
    []any{TokenTypeRawText, "\n    messages.length"},
    []any{TokenTypeRawText, "plural"},
...
result := tokenizeWithoutErrors("{\r\n    messages.length,\r\n    plural,\r\n    =0 {You have \r\nno\r\n messages}\r\n    =1 {One {{message}}}}\r\n", &TokenizeOptions{TokenizeExpansionForms: true, EscapedString: false})
assert.Equal(t, [][]any{
    []any{TokenTypeExpansionFormStart},
    []any{TokenTypeRawText, "\r\n    messages.length"},
    []any{TokenTypeRawText, "plural"},
...
src := "111\n222\n333\nE\n444\n555\n666\n"
file := parse_util.NewParseSourceFile(src, "file://")
location := parse_util.NewParseLocation(file, 12, 123, 456)
span := parse_util.NewParseSourceSpan(location, location, location, nil)
err := parse_util.NewParseError(span, "**ERROR**", nil, nil)
assert.Equal(t, "**ERROR** (\"\n222\n333\n[ERROR ->]E\n444\n555\n\"): file://@123:456", err.Error())
})
})
...
lexerRange := &LexerRange{StartPos: 33, StartLine: 1, StartCol: 10, EndPos: 59}
assert.Equal(t, [][]any{
    []any{TokenTypeText, "line 1\n\"line 2\"\nline 3"},
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts(text, &TokenizeOptions{Range: lexerRange, EscapedString: true}))
assert.Equal(t, [][]any{
    []any{TokenTypeText, "line 1\\n\\\"line 2\\\"\\nline 3"},
    []any{TokenTypeEOF, ""},
}, tokenizeAndHumanizeSourceSpans(text, &TokenizeOptions{Range: lexerRange, EscapedString: true}))
})
...
assert.Equal(t, [][]any{
    ...
    []any{TokenTypeEOF},
}, humanizeParts(Tokenize("<foo [class.text-\nprimary/80]=\"expr\"/>", "someUrl", getHtmlTagDef, nil).Tokens))
})
})
...
t.Run("should treat expansion form as text when they are not parsed", func(t *testing.T) {
    ...
    []any{TokenTypeTagClose, "", "span"},
    []any{TokenTypeEOF},
}, tokenizeAndHumanizeParts("<span>{a, b, =4 {c}}</span>", &TokenizeOptions{TokenizeExpansionForms: false, TokenizeBlocks: &falseVal}))
})
})
...
result := tokenizeAndHumanizeParts("@defer {{one.two, three, =4 {four} =5 {five} foo {bar} }}", &TokenizeOptions{TokenizeExpansionForms: true})
assert.Equal(t, [][]any{
    []any{TokenTypeBlockOpenStart, "defer"},
    []any{TokenTypeBlockOpenEnd},
    []any{TokenTypeExpansionFormStart},
